...
h2. verification that action complete with desirable state in app
...
h2. Interactions with applications
 * clicking
 * visiting
 * drag and drop
 * fill_in forms
...
h2. handle somehow guest type of account

h2.  sessions (from browsers perspective)
   * be default share the same sessions
   * is possible that they have exclusive session ( when testing multiuser interactions)


h2. shared examples that are skeleton for testing
 * pagination
 * sorting
 * ...
...
h2. shortcuts to abstract some internal elements
long class names, constants etc


h2. see partial elemnts in what scope you are
print debug.send(:scopes).last.native

h2. see sorted columns
