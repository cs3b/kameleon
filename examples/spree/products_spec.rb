#https://github.com/spree/spree/blob/master/core/spec/requests/products_spec.rb

require 'spec_helper'

feature "Visiting Products", :driver => :selenium do
  include_context "custom products"

  background do
     @guest = TestUserGuest.new(self) do
      visit spree.root_path
     end
  end

  it "should be able to show the shopping cart after adding a product to it" do
    @guest.click "Ruby on Rails Ringer T-Shirt"
    @guest.see "$17.99"
    @guest.click 'add-to-cart-button'
    @guest.see "Shopping Cart"
  end

  it "should be able to search for a product" do
    @guest.fill_in("shirt" => "keywords")
    @guest.click "Search"
  ...
  it "should be able to visit brand Ruby on Rails" do
    @guest.within('#taxonomies') { click "Ruby on Rails" }
  ...
  it "should be able to visit brand Ruby" do
    @guest.within('#taxonomies') { click "Ruby" }
  ...
  it "should be able to visit brand Apache" do
    @guest.within('#taxonomies') { click "Apache" }
  ...
  it "should be able to display products priced under 10 dollars" do
    ...
    @guest.within('#sidebar_products_search') { click "Search" }
    @guest.see "No products found"
  end

  it "should be able to display products priced between 15 and 18 dollars" do
    @guest.within('#taxonomies') { click "Ruby on Rails" }
    @guest.fill_in(:check =>  "Price_Range_$15.00_-_$18.00" )
    @guest.within('#sidebar_products_search') { click "Search" }
  ...
  it "should be able to display products priced 18 dollars and above" do
    @guest.within('#taxonomies') { click "Ruby on Rails" }
    @guest.fill_in(:check => "Price_Range_$18.00_-_$20.00")
    @guest.fill_in(:check => "Price_Range_$20.00_or_over")
    @guest.within('#sidebar_products_search') { click "Search" }
